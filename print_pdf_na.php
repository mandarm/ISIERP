...
<?php 
	...
	if (isset($_POST['print_stud_list']) and $_POST['order_by']=='Subject') {
		...
		$pdf->Cell(0,7,"THIS LIST IS NOT FINAL",0,1,'C');
		$pdf->Cell(0,7,"THIS IS FOR MOMENTARY INFORMATION ONLY",0,1,'C');
		...
		//-------------------Reprint Header end--------------------------------------------------
        	...
        	$status=$irec['registration_verification_status'];
...
<?php 
	...
	if (isset($_POST['print_stud_list']) and $_POST['order_by']=='Student') {
		...
		$sql1 = "SELECT * FROM grades where course='$crs' and batch='$bt' and semester='$sem' and roll_no='$student_roll' order by subject_type,pool,subject_name";                           
        $stmt1 = $pdo->prepare($sql1);
        $stmt1->execute();
        $i=1;
        while ($irec = $stmt1->fetch(PDO::FETCH_ASSOC)) {
